...
\section{Introduction}
	BeviMed~\cite{bevimed} is a procedure for evaluating the evidence of association between allele configurations across rare variants within a genomic locus and a case/control label.
	...
	Under the model labelled $\gamma = 1$, the probability of case status is linked to the configuration of alleles, and a latent partition of variants into \emph{pathogenic} and \emph{non-pathogenic} groups conditional on a mode of inheritance.  It can also perform model selection on multiple models of association corresponding to different classes of variants.

	The aim of the package is to facilitate prioritisation of large numbers of loci and the variants therein by rapid inference of the posterior distributions of $\gamma$, mode of inheritance parameter $m$, and indicator of pathogenicity across variants, $z$. 
	...
	See the `BeviMed Introduction' package for a quick start guide. 
...
In this example we use \texttt{bevimed\_polytomous} to compare two models in the situation where the disease status depends on only the allele configuration at the first variant site from amongst \Sexpr{ncol(G)} columns: one depending on just first variant and one depending on all of the variants. 
...
The inference functions \texttt{bevimed\_m}, \texttt{bevimed} and \texttt{bevimed\_polytomous} always sample from the posterior distribution of pathogenicity of each variant represented in allele count matrix $G$. However, often $G$ does not all contain information about whether all variants represented are pathogenic. This occurs when alleles for the variant are either not observed (i.e. the allele count is zero for all individuals) or when alleles for variant are only present in individuals whose total allele count is less than their ploidy. The function \texttt{subset\_variants} can be used to remove such variants, returning either a transformed matrix or the indices of the remaining variants within the original set. Typically, $G$ would only contain variants which were observed in at least one of the individuals, so using this function \emph{a priori} is not likely to result in a speed up when applied conditioning on dominant inheritance, but often only a relatively small number of variants are observed are observed in compound heterozygotes/homozygotes and therefore is likely to result in a speed up conditioning on recessive inheritance.
...
\newblock Am. J. Hum. Genet. doi: 10.1016/j.ajhg.2017.05.015.
...
\newblock Nat Genet. Feb 2. doi: 10.1038/ng.2892.
